...
# 统计label出现次数
path1 = os.getcwd()+'\\watermelon.csv'
...
def get_max_gain(temp_data):
    columns_entropy = [(col, calcu_each_gain(temp_data[col], temp_data))
                       for col in temp_data.iloc[:, :-1]]
...
def get_most_label(label_list):
    ...
    sorted_label = sorted(label_dict.items(),
                          key=lambda ll: ll[1], reverse=True)
...
def plot_mid_text(cntr_pt, parent_pt, txt_str):
    ...
    create_plot.ax1.text(x_mid, y_mid, txt_str, va="center",
                         ha="center", color='red', FontProperties=font)
...
def plot_tree(decision_tree, parent_pt, node_txt):
    ...
    first_str = next(iter(decision_tree))
    cntr_pt = (plot_tree.xoff + (1.0 + float(num_leafs)) /
               2.0/plot_tree.totalW, plot_tree.yoff)
    ...
    for k in second_dict.keys():
        if isinstance(second_dict[k], dict):
            ...
            plot_node(second_dict[k], (plot_tree.xoff,
                                       plot_tree.yoff), cntr_pt, leaf_node)
...
if __name__ == '__main__':
    my_data = pd.read_csv(f1)
    column_count = dict([(ds, list(pd.unique(my_data[ds])))
                         for ds in my_data.iloc[:, :-1].columns])
